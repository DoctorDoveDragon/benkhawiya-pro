...
async def api_root():
    ...
    return {
        ...
        "features": {
            "council_reasoning": True,
            "cosmic_principles": True,
...
async def get_cosmic_principles(
    aspect: CouncilAspect = None,
    engine: BenkhawiyaEngine = Depends(get_cosmic_engine)
) -> Dict[str, Any]:
    principles = (
        engine.get_principles_by_aspect(aspect) 
        if aspect else engine.principles
    )
    return {
...
async def calculate_golden_progression(
    n: int,
    engine: BenkhawiyaEngine = Depends(get_cosmic_engine)
) -> Dict[str, Any]:
    if n < 0 or n > 100:
